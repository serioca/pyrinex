...
# %% test reading all satellites
    for u in (None, 'm', 'all', ' ', '', ['G', 'R', 'S']):
        print('use', u)
        obs = pr.rinexobs(rdir/'demo.10o', use=u)
        assert obs.equals(truth)

# %% test read .nc
    print('load NetCDF via API')
    obs = pr.rinexobs(rdir/'test2all.nc')
...
# %% test write .nc
    ...
    with tempfile.TemporaryDirectory() as d:
        obs = pr.rinexobs(rdir/'demo.10o', ofn=Path(d)/'testout.nc')
...
def test_obs2_multisat():
    ...
    obs = pr.rinexobs(rdir/'demo.10o', use=('G', 'R'))
    assert obs.equals(truth)
